...
// Phase 37: bigger model. V=27, E=64, HID=192, CTX=2 → P = 31 707 (13× the
// toy). The whole point: at this scale, random flip-and-accept's 6-of-P
// selection should stall — there are too many parameters to hit a useful
// one by chance — while SPSA's coherent direction step still descends.
const V = 27;             // a-z + space
const E = 64;             // embed dim per position (was 16)
const HID = 192;          // hidden width (was 32)
const CTX = 2;            // context length (previous CTX chars predict the next)
const P_EMBED = V * E;                  // 1728
const P_FC1 = CTX * E * HID;            // 24576
const P_B1 = HID;                       // 192
const P_FC2 = HID * V;                  // 5184
const P_B2 = V;                         // 27
const P = P_EMBED + P_FC1 + P_B1 + P_FC2 + P_B2;   // 31707
const TARGET_PROPOSALS = 2;
const FLIP_SIZE = 6;
const FLIP_SIGMA = 0.15;
const SNAPSHOT_EVERY = 50;
const SNAPSHOT_KEY_PREFIX = "lm-big/";
...
export class TournamentLMBig extends DurableObject<Env> {
  private round = 0;
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (url.pathname === "/api/lm-big/tick" && request.method === "POST") return this.tick(request);
    if (url.pathname === "/api/lm-big/state") return Response.json(this.summary());
    if (url.pathname === "/api/lm-big/sample") {
    ...
    if (url.pathname === "/api/lm-big/snapshot") {
      ...
      for (let k = 0; k < numShards; k++) {
        ...
        const headerSize = k === 0 ? 8 : 0;
        shards.push({
          url: `/api/lm-big/snapshot.bin?round=${round}&shard=${k}`,
          shard: k,
    ...
    if (url.pathname === "/api/lm-big/snapshot.bin") {
    ...
    if (url.pathname === "/api/lm-big/reset" && request.method === "POST") {
      await this.resetState();
      return Response.json({ ok: true });
    }
    if (url.pathname === "/api/lm-big/ws") {
