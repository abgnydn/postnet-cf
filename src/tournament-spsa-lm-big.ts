...
// Phase 37: bigger architecture, identical to TournamentLMBig. The pair
// (lm-big, spsa-lm-big) is what tests the scaling claim — that random
// flip-and-accept stalls past P ~ 10K while SPSA still descends.
const V = 27;
const E = 64;        // was 16
const HID = 192;     // was 32
const CTX = 2;
...
const SNAPSHOT_KEY_PREFIX = "spsa-lm-big/";
...
export class TournamentSpsaLMBig extends DurableObject<Env> {
  private round = 0;
  ...
  async fetch(request: Request): Promise<Response> {
    ...
    if (url.pathname === "/api/spsa-lm-big/tick" && request.method === "POST") return this.tick(request);
    if (url.pathname === "/api/spsa-lm-big/state") return Response.json(this.summary());
    if (url.pathname === "/api/spsa-lm-big/sample") {
    ...
    if (url.pathname === "/api/spsa-lm-big/snapshot") {
      ...
      for (let k = 0; k < numShards; k++) {
        ...
        const headerSize = k === 0 ? 8 : 0;
        shards.push({
          url: `/api/spsa-lm-big/snapshot.bin?round=${round}&shard=${k}`,
          shard: k,
    ...
    if (url.pathname === "/api/spsa-lm-big/snapshot.bin") {
    ...
    if (url.pathname === "/api/spsa-lm-big/reset" && request.method === "POST") {
      await this.resetState();
      return Response.json({ ok: true });
    }
    if (url.pathname === "/api/spsa-lm-big/ws") {
