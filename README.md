...
| 40-4b-s5 | **It works.** optimum-cli + `scripts/inject-gates-onnx.py` + int8 quantize → 994 MB single-file ONNX. Chrome tab loads it, ORT-web runs forward + SPSA loop end-to-end. R=4: loss 4.10→3.85, sym-AIMD η grew 1.0e-3→1.1e-3 | `5276b96` |
| 40-4b-s5b | Polish + protocol finding — attack mode wired (saves ~2 GB on attacker tab by skipping ONNX), WebGPU EP w/ WASM fallback, `Response.bytes()` for single-allocation model load, TARGET=2. Live-tested byzantine defense; surfaced that Phase 40-3's trusted-auditor scheme degrades when honest:attacker compute ratio is large (1.5% fraud rate vs 40% threshold) | _uncommitted_ |
| 40-4b-s6 | Closed the byzantine hole — no-self-audit queue (`pendingAudits[]`), reject `audit_loss_before ≤ 0`, magnitude-lie fraud test (`claimedΔ < realΔ − 0.5` flags exaggerated improvement claims missed by Phase 39's inversion test). Live two-tab Chrome verified: cross-audit ledger grows correctly under attacker spam, no self-closure possible | `4ff5190` |
| 40-4b-s6e | Empirical — python honest verifier (Qwen on MPS) + browser attacker tab over R=183. Attacker quarantined at 10 audits (100% fraud rate via magnitude check). Post-quarantine loss 1.7632 → 1.7570 with η climbing 1.0e-3 → 2.8e-3 (21 grow, 0 shrink). 20-byte wire, attacker active throughout (see `docs/PHASE_40_NEXT6_EMPIRICAL.md`) | `f791d19` |
| 40-4b-s6e-multi | Multi-seed sweep — 5 runs (seeds 1–5) on Kaggle T4 against the deployed Worker, python honest + python attacker. **5/5 quarantine** at attacker fraud rate **1.000 ± 0.000**. Final loss **1.762674 ± 0.000294** (σ below per-round Δloss magnitude). Descent reproducible across seeds; η drift varies more (grow 5.0 ± 2.5) but loss outcome is invariant. Notebook: `notebooks/phase40_next6e_sweep_kaggle.ipynb` | _uncommitted_ |
| 40-4b-s7 | HF Hub hosting — worker `ONNX_URL` now resolves `?onnx=` → localhost `:8788` (dev) → **HuggingFace Hub** (prod default), so the deployed demo needs no local server. Self-contained `scripts/upload-onnx-hf.py` (uv, cached HF token) creates `abgunaydin/postnet-qwen05b-with-gates` + model card and publishes the 994 MB int8 ONNX. Code + uploader shipped and dry-run-verified (auth ✓); the one-time upload runs from the machine holding the file | _uncommitted_ |
