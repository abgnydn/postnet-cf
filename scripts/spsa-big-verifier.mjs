...
// Phase 37: big variant. Must match src/tournament-spsa-lm-big.ts
const V = 27, E = 64, HID = 192, CTX = 2;
const P_EMBED = V * E;
...
class SpsaWorker {
  ...
  async bootstrap() {
    const meta = await this.fetchJson(`${COORD}/api/spsa-lm-big/snapshot`);
  ...
  async tick(round, seed, scalar_g, delta) {
    ...
    return await this.fetchJson(`${COORD}/api/spsa-lm-big/tick`, {
      method: "POST",
...
(async () => {
  ...
  await fetch(`${COORD}/api/spsa-lm-big/reset`, { method: "POST" });
  const workerIds = ["alpha", "delta", "bravo"];
  if (process.env.BYZANTINE === "1") workerIds.push("byz");
  const workers = workerIds.map(suffix => new SpsaWorker(`spsa-lm-big-${suffix}`));
  ...
  const sampler = (async () => {
    while (true) {
      const s = await (await fetch(`${COORD}/api/spsa-lm-big/state`)).json();
