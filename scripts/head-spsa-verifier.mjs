// Phase 38 — head-classifier SPSA verifier.
// Same task as head-flip-verifier.mjs but DeComFL-style proposal shape:
// (seed, scalar_g, delta) → server reconstructs u and applies θ ← θ − η·g·u.
...
class SpsaWorker {
  ...
  async bootstrap() {
    const meta = await this.fetchJson(`${COORD}/api/head-spsa/snapshot`);
  ...
  async tick(round, seed, scalar_g, delta) {
    ...
    return await this.fetchJson(`${COORD}/api/head-spsa/tick`, {
      method: "POST",
...
(async () => {
  ...
  await fetch(`${COORD}/api/head-spsa/reset`, { method: "POST" });
  ...
  const workers = workerIds.map((suffix, i) => new SpsaWorker(
    `head-spsa-${suffix}`, shardForWorkerIdx(i % N_WORKERS), data.features, data.labels));
  ...
  const sampler = (async () => {
    while (true) {
      const s = await (await fetch(`${COORD}/api/head-spsa/state`)).json();
