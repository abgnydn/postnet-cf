...
// Phase 37: big variant. Must match src/tournament-lm-big.ts
...
class Worker {
  ...
  async bootstrap() {
    ...
    const meta = await this.fetchJson(`${COORD}/api/lm-big/snapshot`);
  ...
  async tick(round, indices, values, delta) {
    ...
    return await this.fetchJson(`${COORD}/api/lm-big/tick`, {
      method: "POST",
...
(async () => {
  ...
  await fetch(`${COORD}/api/lm-big/reset`, { method: "POST" });
  ...
  const workers = workerIds.map(suffix => new Worker(`lm-big-${suffix}`));
  ...
  const sampler = (async () => {
    while (true) {
      const s = await (await fetch(`${COORD}/api/lm-big/state`)).json();
